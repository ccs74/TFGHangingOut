...
@Component({
  selector: 'app-actividades-asistidas',
  templateUrl: './actividades-asistidas.component.html',
  styleUrls: ['./actividades-asistidas.component.css']
})
export class ActividadesAsistidasComponent implements OnInit {
  ...
  cambiarPag(){
    localStorage.setItem('before', `/actividades/asistidas`);
  }
  cargarActividades(){
    ...
    .subscribe( ( res: any) => {
      ...
      for(let i = 0; i < res['actividades'].length; i++){
        if(res['actividades'][i].participantes.length > 0){
          for(let j = 0; j < res['actividades'][i].participantes.length; j++){
            if(res['actividades'][i].participantes[j] == this.idUsuario){
...
function convertirFecha2(fecha: Date) {
